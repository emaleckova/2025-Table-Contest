...
load(file = "helpers/Global_objects.rds")
```
...
2. We will create a simple `gt` table
...
3. We need to convert the `gt` table to a _grob_ (required by `{patchwork}`), which requires a few dependencies
