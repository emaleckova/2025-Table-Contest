...
PlotCountry <- function(target_country) {
  ...
  p_cont <- ggplot() +
    geom_sf(data = vegetation_sf_continents[vegetation_sf_continents$continent == target_continent, ], fill = "white") +
    geom_sf(data = vegetation_sf_countries[vegetation_sf_countries$name_en == target_country, ], fill = "blue") +
...
PlotHalfViolins <- function(target_country){
  p_halfv <- ggplot(filter(dat_vegetation, country == target_country), aes(x = country, y = delta)) +
    ...
    geom_jitter(aes(fill = type), alpha = 0.8, width = 0.15) +
...
dat_vegetation |>
  ...
  text_transform(
    ...
    fn = function(x) {
      lapply(x, function(y) {
        html(PlotCountry(y) |> ggplot_image(height = px(250), aspect_ratio = 3))
      })
    }
  ) |> 
  ...
  text_transform(
    ...
    fn = function(column) {
      map(column, PlotHalfViolins) |>
        ggplot_image(height = px(250), aspect_ratio = 1)
    }
  ) |> 
