...
PlotCountry <- function(target_country) {
  ...
  p_cont <- ggplot() +
    geom_sf(data = vegetation_sf_continents[vegetation_sf_continents$continent == target_continent, ], fill = "white") +
    geom_sf(data = vegetation_sf_countries[vegetation_sf_countries$name_en == target_country, ], fill = "#6A4531FF") +
...
PlotHalfViolins <- function(target_country){
  p_halfv <- ggplot(filter(dat_vegetation, country == target_country), aes(x = country, y = delta)) +
    geom_violinhalf(colour = "#6A4531FF", fill = "#A87250FF", position = position_nudge(x = 0.05, y = 0)) +
    geom_jitter(aes(fill = type), colour = "#6A4531FF", alpha = 0.8, width = 0.01) +
...
dat_vegetation |>
  ...
  # placeholder column -> violin plots come here
  ...
  text_transform(
    ...
    fn = function(x) {
      lapply(x, function(y) {
        html(PlotCountry(y) |> ggplot_image(height = px(250), aspect_ratio = 1))
      })
    }
  ) |>
  ...
  text_transform(
    ...
    fn = function(column) {
      map(column, PlotHalfViolins) |>
        ggplot_image(height = px(250), aspect_ratio = 0.3)
    }
  ) |>
  tab_style(
    style = list(
...
opt_css(css ="
    #countries_tbl .gt_table {
      background: linear-gradient(180deg, #E7A974, #FDDDAB);
    }
    
    #countries_tbl .gt_col_heading, #countries_tbl .gt_footnotes, #countries_tbl .gt_column_spanner_outer, #countries_tbl .gt_row.gt_left.gt_stub_row_group {
      background:transparent; border-right-style: none;
    }") |>
