...
PlotHalfViolins <- function(data_for_plot) {
  p_halfv <- ggplot(data_for_plot, aes(x = country, y = delta)) +
...
summarized_tbl_input |> 
  ...
  text_transform(
    ...
    fn = function(x) {
      ...
      map2(x, 1:length(x), function(col_value, i) {
        ...
        data_for_plot <- dat_vegetation |>
          filter(country == !!target_country, type == !!target_type)

        PlotHalfViolins(data_for_plot) |>
  ...
  opt_css(css ="
    ...
    #countries_tbl .gt_col_heading, #countries_tbl .gt_column_spanner_outer, #countries_tbl .gt_row.gt_left.gt_stub, #countries_tbl .gt_row.gt_left.gt_stub_row_group {
...
PlotGirafeViolins <- function(target_country){
